...
- **🧑‍💼 Chief of Staff 线程**：你的 AI 参谋长，每天帮你过滤 Slack / 邮件，告诉你今天最重要的 3 件事。
- **📦 Release 线程**：专门盯着这次发版，记录所有决策、回归问题、上线检查项。
- **📚 文档评审线程**：长期跟进一份大文档的修订，每次回来都能从上次中断的地方继续。
- **📡 外部监控线程**：盯一个竞争对手网站、监控一个 API、追踪某条新闻线索。
...
| 操作 | Steering（操控） | Queuing（排队） |
|---|---|---|
| **何时触发** | AI 当前任务进行中 | 当前任务结束后 |
| **作用** | **打断**它，纠正方向 | **追加**下一个任务 |
| **典型用法** | “这块再小一点”<br>“间距感觉不对”<br>“这句文案错了” | “做完之后把预览链接发给评审人 Slack” |
| **心智模型** | 改变它**正在干**的事 | 规划它**接下来**要干的事 |
...
- **🪟 `$browser`**：应用内置浏览器，在侧边栏里直接审查、标注网页。
- **🌍 `@chrome`**：基于你已登录的 Chrome 状态，处理需要登录态的浏览器流程。
- **🖥️ `@computer`**：直接控制桌面 GUI，搞那些只能靠点鼠标完成的事。
...
| 新原语 | 它做了什么 |
|---|---|
| 🎯 Steering 操控 | 打断**正在进行**的工作 |
| 📋 Queuing 排队 | 规划**接下来要做**的工作 |
| ⏰ Thread Automations | 你**离开**时让线程继续活着 |
| 🏁 Goals 目标 | 给一条 Codex **可以反复冲击**的终点线 |
